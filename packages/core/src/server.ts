...
class Server {
  ...
  private async initializePlugins(): Promise<void> {
    const plugins = this.configService.get("plugins") as Array<{
      name: string;
      enabled?: boolean;
    ...
    for (const pluginConfig of plugins) {
      ...
      try {
        if (pluginConfig.name === 'token-speed') {
          pluginManager.registerPlugin(tokenSpeedPlugin, {
            enabled: true,
            options: pluginConfig.options || { reporter: ['console'] }
          });
          await pluginManager.enablePlugin('token-speed', this.app);
          this.app.log.info('✅ Token speed plugin enabled');
        }
        // 可以在这里添加更多插件
      } catch (error) {
        const errMsg = error instanceof Error ? error.message : String(error);
        this.app.log.error(`Failed to enable plugin ${pluginConfig.name}: ${errMsg}`);
      }
    }
  }
