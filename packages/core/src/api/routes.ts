...
async function handleFallback(
  ...
  req.log.warn(
    `Request failed for ${(req as any).scenarioType}, trying ${fallbackList.length} fallback models`,
  );
  ...
  for (const fallbackModel of fallbackList) {
    try {
      ...
      req.log.warn(
        `Fallback model ${fallbackModel} failed: ${fallbackError.message}`,
      );
      continue;
    }
  }
...
function shouldBypassTransformers(
  ...
  // Passthrough transformer: always bypass all transformations
  if (transformer.name === "Passthrough") {
    return true;
  }
