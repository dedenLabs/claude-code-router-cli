    ...
    fastify.addHook('onSend', async (request, _reply, payload) => {
      ...
      if (payload instanceof ReadableStream) {
        ...
        const processStats = async () => {
          ...
          let lastOutputTokenCount = 0;  // Track last output token count to avoid duplicate 0 tps logs
          ...
          const doOutput = async (isFinal: boolean) => {
            ...
            if (!isFinal) {
              ...
              // Skip output if token count hasn't changed (no new data)
              if (stats.tokenCount === lastOutputTokenCount) {
                return;
              }
              lastOutputTokenCount = stats.tokenCount;
            } else {
...
async function outputStats(
  ...
  const prefix = isFinal ? '[Token 速度]' : '[Token 速度]';

  // Parse timeToFirstToken to number if it's in ms format
  let ttftValue: number | string = 'N/A';
  if (stats.timeToFirstToken) {
    ttftValue = stats.timeToFirstToken;
  }
  ...
  const logData = {
    ...
    tps: stats.tokensPerSecond,  // Alias for easier detection
